...
5. Use the `read_excel()` function in the `readxl` package to read the `Monuments.xlsx` file and call the output `mon`. 

```{r}
mon <- read_excel(path = "~/Downloads/intro_to_r/modules/Data_IO/lab/Monuments.xlsx")
```
